...
colnames(df[1:40])
df_prediction = cbind(df[3], df[9:31])
df_prediction %>% str()
...
df_prediction$Buyers_vs_NonBuyers = as.character(df_prediction$Buyers_vs_NonBuyers)
df_prediction$Buyers_vs_NonBuyers = ifelse(df_prediction$Buyers_vs_NonBuyers == "Buyers",1, 0)
df_prediction$Buyers_vs_NonBuyers = factor(df_prediction$Buyers_vs_NonBuyers, levels = c(0, 1))
...
## Assumption
### Cost of missing out customer who will buy = 5 times
cost_mat = as.data.frame(rbind(c(0,0,0), c(1, 0, 5), c(0, 1, 1), c(1,1,0)))

## Random Forest
rf1 = randomForest(Buyers_vs_NonBuyers~., data = df_prediction_train)
varImpPlot(rf1, sort = T, main = "Buyer  vs No Buyer", n.var = 6)

pred_rf1 = predict(object = rf1, newdata = df_prediction_test, type = "prob")
pred_rf1 = as.data.frame(pred_rf1)

pred_rf_table = data.frame(table(df_prediction_test$Buyers_vs_NonBuyers, ifelse(pred_rf1$`1` >= 0.5, 1,0)))
...
cost_fn() #Cost of Model for Buy Vs NoBuy
...
metrics_fn()#Sensitivity, Specificity

## Logistic Regression
logistic1 = glm(Buyers_vs_NonBuyers~., data = df_prediction_train, family = binomial)
pred_logistic1 = predict(logistic1, newdata = df_prediction_test, type = "response")
summary(logistic1)
varImp(logistic1)

pred_log1 = as.data.frame(table(df_prediction_test$Buyers_vs_NonBuyers, ifelse(pred_logistic1 >= 0.5, 1,0)))
...
logistic2 = glm(Buyers_vs_NonBuyers~ Q1_Age + Q28_31_Information_Sources + Q24_Total_Investable_Assets + Q25_Total_Household_Debt_Estimate + Q20_Insured + Q8_Annual_Household_Income + Q26_People + Q30_Event + Q12_Education_Level, family = "binomial", data = df_prediction_train)

pred_logistic2 = predict(logistic2, df_prediction_test, type = "response")

pred_log2 = data.frame(table(df_prediction_test$Buyers_vs_NonBuyers, ifelse(pred_logistic2 >= 0.5, 1,0)))

pred_log2 = apply(pred_log2, MARGIN = 2, function(x) as.integer(as.character(x)))
pred_log2 = data.frame(pred_log2)
cost_fn(pred_table = pred_log2)#Not required
...
rf2 = train(Buyers_vs_NonBuyers~ ., data = df_prediction_train, method = "rf", trControl = objControl)

varImp(rf2)

pred_rf2 = predict(object = rf2, newdata = df_prediction_test)
pred_rf2_table = data.frame(table(df_prediction_test$Buyers_vs_NonBuyers, pred_rf2))
...
gam1 = gam(Buyers_vs_NonBuyers~ s(Q1_Age) + s(Q28_31_Information_Sources) + s(Q24_Total_Investable_Assets) + s(Q25_Total_Household_Debt_Estimate) + Q20_Insured + s(Q8_Annual_Household_Income), family = "binomial", data = df_prediction_train)
plot.gam(gam1,rugplot = F, col = "deepskyblue3")

pred_gam = predict(gam1, newdata = df_prediction_test, type = "response")
pred_gam_table = data.frame(table(df_prediction_test$Buyers_vs_NonBuyers, ifelse(pred_gam >= 0.5, 1,0)))
